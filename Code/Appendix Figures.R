...
plot_bias_proportion <- function(result) {
  ...
  highlight_data <- result[
    result$specificity > 0.9 & result$sensitivity > 0.8,
  ]
...
# hybrid_KDCA <- hybrid_KDCA %>% filter(latest_immunology_cat %in% c("< 1 month", "1-6 months")) # figS2(1).png
...
png("../result/plots/figS2(2).png", width = 2400, height = 1600, res = 350)
par(mar = c(4, 4, 1, 1))
plot_bias_proportion(results)
dev.off()
